...
public class Dictionnaire extends Observable {
	...
	public int chercherIndiceVariableSortante(int jE)  {
		...
		for (int i=1; i<=nbVarBase; i++){
			if(dict[i][jE] < 0){
				double rapport =-dict[i][0]/dict[i][jE]; 
				if (min > rapport){
					min = rapport;
					iS = i;
				}
		...
		// A modifier
		return (min < Double.MAX_VALUE) ? iS : 0 ; 
	}
	...
	public void pivoter(int iS, int jE) {
		double pivot = 1/dict[iS][jE];
		for (int j=0; j<=nbVarHorsBase; j++){
			if (j!=jE){
				dict[iS][j]/=-dict[iS][jE];
			}
		}
		dict[iS][jE]=pivot;
		for (int i=0; i<=nbVarBase;i++){
			if(i!=iS){
				for (int j=0; j<=nbVarHorsBase;j++){
					if (j!=jE)
					dict[i][j]+=dict[i][jE]*dict[iS][j];
				}
				dict[i][jE] *= pivot; 
			}
		}
		int i_tmp = tabVarBase[iS];
		int j_tmp = tabVarHorsBase[jE];
		tabVarBase[iS]=j_tmp;
		tabVarHorsBase[jE]=i_tmp;
	}
